...
export METADATA_STORE_DOMAIN=metadata-store.${INGRESS_DOMAIN}
export AUTH_TOKEN=$(kubectl get secret -n metadata-store metadata-store-read-only-token -otemplate='{{.data.token | base64decode}}')
